<div class="span12">
  <div class="widget widget-nopad">
    <div class="widget-header"> <i class="fa fa-list-alt"></i>
      <h3> {{{ Lang::get('admin/admin.today_status') }}} </h3>
    </div>
    <!-- /widget-header -->
    <div class="widget-content">
      <div class="widget big-stats-container">
        <div class="widget-content">
          <h6 class="bigstats">显示当前网站数据量, 最后更新时间 {{{date("Y-m-d") }}}</h6>
          <div id="big_stats" class="cf">
            <div class="stat"> <i class="fa fa-users"></i> <div>{{{ Lang::get('admin/admin.register_users') }}}</div><span class="value">851</span> </div>
            <!-- .stat -->
            
            <div class="stat"> <i class="fa fa-map-marker"></i> <div>{{{ Lang::get('admin/admin.posts') }}}</div><span class="value">423</span> </div>
            <!-- .stat -->
            
            <div class="stat"> <i class="fa fa-file-image-o"></i> <div>{{{ Lang::get('admin/admin.attachment') }}}</div><span class="value">922</span> </div>
            <!-- .stat -->
            
            <div class="stat"> <i class="fa fa-comments"></i> <div>{{{ Lang::get('admin/admin.comments') }}}</div><span class="value">851</span> </div>
            <!-- .stat --> 
          </div>
          <hr>
          <div id="big_stats" class="cf">
            <div class="stat"> <i class="fa fa-globe"></i> <div>{{{ Lang::get('admin/admin.visits') }}}</div><span class="value">851</span> </div>
            <!-- .stat -->
            
            <div class="stat"> <i class="fa fa-clock-o"></i> <div>{{{ Lang::get('admin/admin.visit_time') }}}</div><span class="value">423</span> </div>
            <!-- .stat -->
            
            <div class="stat"> <i class="fa fa-male"></i> <div>{{{ Lang::get('admin/admin.online') }}}</div><span class="value">922</span> </div>
            <!-- .stat -->
            
            <div class="stat"> <i class="fa fa-thumbs-down"></i> <div>{{{ Lang::get('admin/admin.complaints') }}}</div><span class="value">851</span> </div>
            <!-- .stat --> 
          </div>
        </div>
        <!-- /widget-content --> 
